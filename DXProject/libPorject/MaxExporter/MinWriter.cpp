...
void MinWriter::Set(const TCHAR * name, Interface * pMax)
{
	m_pMax = pMax;
	m_filename = name;
...
void MinWriter::PreProcess(INode * pNode)
{
...
void MinWriter::AddObject(INode * pNode)
{
	//GEOMOBJECT_CLASS_ID;
	//HELPER_CLASS_ID;
...
UINT MinWriter::CheckMVERTEX(vector<MVERTEX> &list, MVERTEX data)
{
...
UINT MinWriter::CheckCVERTEX(vector<CVERTEX> &list, CVERTEX data)
{
	for (int i = 0; i < list.size(); i++)
	{
		if (list[i].p == data.p && list[i].t == data.t && list[i].n == data.n)
...
TriObject* MinWriter::GetTriObjectFromNode(INode * pNode, TimeValue tick, bool& deleteit)
{
...
void MinWriter::ExportParent(INode * pNode)
{
...
void MinWriter::ExportMatrix(INode * pNode)
{
...
Point3 MinWriter::GetTangetVector(Point3 p0, Point3 p1, Point3 p2, Point3 t0, Point3 t1, Point3 t2, Point3 n1)
{
...
BOOL MinWriter::TMNegParity(Matrix3 & m)
{
	return (DotProd(CrossProd(m.GetRow(0), m.GetRow(1)), m.GetRow(2)) < 0.0) ? 1 : 0;
}

void MinWriter::ExportMesh(INode * pNode)
{
	// ------------------------------------------------------------------------------------------------------------
...
int MinWriter::SearchType(INode * pNode)
{
	ObjectState os = pNode->EvalWorldState(0);
	if (os.obj)
	{
...
void MinWriter::ExportSkinningMesh(INode * pNode)
{
	Modifier* skinMod = FindModifyer(pNode, SKIN_CLASSID);
...
void MinWriter::ExportSkinningMesh2(INode * pNode)
{
	Modifier* skinMod = FindModifyer(pNode, SKIN_CLASSID);
	if (skinMod == nullptr) return;
	//get the skin interface
	...
	//	맥스에서는 4 x 3 행렬(Matrix3)을 사용한다.
	// ------------------------------------------------------------------------------------------------------------
	Matrix3 wtm = pNode->GetObjTMAfterWSM(0);
	Matrix3 tm = pNode->GetNodeTM(0);
	BOOL negScale = TMNegParity(tm);
	bool deleteit = false;
	Inverse(tm, tm);
	...
	if (mesh)
	{
		...
		for (int i = 0; i < mesh->numVerts; i++)
		{
			Point3 V = mesh->verts[i] * wtm;

			Point3 tempV = Point3(0, 0, 0);

			int numOfWeights = skinData->GetNumAssignedBones(i);
			float4 f4Weight(0, 0, 0, 0);

			for (int j = 0; j < mmin(numOfWeights, 4); j++)
			{
				f4Weight.v[j] = skinData->GetBoneWeight(i, j);
			}
			if (numOfWeights > 4)
			{
				f4Weight.Normalize();
			...
			for (int j = 0; j < numOfWeights; j++)
			{
				...
				if (pBone != NULL)
				{
					tempV += (Inverse(pBone->GetNodeTM(0)) * V) * f4Weight.v[j];
					//tempV = V * Inverse(pBone->GetNodeTM(0));
				}
			}
			V = tempV;
...
void MinWriter::ExportMaterial(INode * pNode)
{
...
void MinWriter::ExportAnimationKeys(INode * pNode)
{
	// 스케일은 나중에 추가
...
void MinWriter::SwapFloat(float & data1, float & data2)
{
	float temp = data1;
	data1 = data2;
	data2 = temp;
...
Point3 MinWriter::GetVertexNormal(Mesh * mesh, int faceNo, RVertex * rv)
{
...
UINT MinWriter::CheckVertexNormal(vector<Point3>& list, Point3 data)
{
	for(int i = 0; i < list.size(); i++)
	{
...
bool MinWriter::Export()
{
	...
	for (int i = 0; i < m_ObjectList.size(); i++)
	{
		...
		{
		case 1:
			_ftprintf(m_pStream, L"TYPE GEOMETRY\n");
			break;
		case 2:
			_ftprintf(m_pStream, L"TYPE SKINGEOMETRY\n");
			break;
		case 3:
