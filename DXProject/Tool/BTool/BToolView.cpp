...
void CBToolView::OnSize(UINT nType, int cx, int cy)
{
	CView::OnSize(nType, cx, cy);

	// TODO: 여기에 메시지 처리기 코드를 추가합니다.
	CBToolApp* pApp = (CBToolApp*)AfxGetApp();
	pApp->m_Tool.ResizeDevice(cx, cy);
}


LRESULT CBToolView::WindowProc(MSG msg)
{
	// TODO: 여기에 특수화된 코드를 추가 및/또는 기본 클래스를 호출합니다.
	CBToolApp* pApp = (CBToolApp*)AfxGetApp();
	HWND hWnd = pApp->m_Tool.m_hWnd;
	pApp->m_Tool.MCoreMsgProc(msg);
	return CView::WindowProc(msg.message, msg.wParam, msg.lParam);
}
