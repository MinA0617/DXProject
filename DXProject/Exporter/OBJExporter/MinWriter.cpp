...
void MOBJWriter::ExportBoundingbox(INode * pNode)
{
	...
	Point3 min3 = box.pmin; //*Inverse(tm);
	Point3 max3 = box.pmax; //*Inverse(tm);
	SwapFloat(min3.y, min3.z);
	SwapFloat(max3.y, max3.z);
	_ftprintf(m_pStream, L"BOUNDING_BOX\n");
	_ftprintf(m_pStream, L"%10.4f %10.4f %10.4f", min3.x, min3.y, min3.z);
	_ftprintf(m_pStream, L"%10.4f %10.4f %10.4f", max3.x, max3.y, max3.z);
	_ftprintf(m_pStream, L"\n");
}

void MOBJWriter::ExportSkinBoundingbox(INode * pNode)
{
	Matrix3 tm = pNode->GetNodeTM(0);
	bool deleteit = false;
	TriObject* tri = GetTriObjectFromNode(pNode, 0, deleteit);
	...
	float minp = min(min3.x, min3.z) * 1.1;
	float maxp = max(max3.x, max3.z) * 1.1;
...
bool MOBJWriter::Export()
{
	...
	for (int i = 0; i < m_ObjectList.size(); i++)
	{
		...
		case 2:
			...
			ExportMaterial(m_ObjectList[i]);
			ExportSkinBoundingbox(m_ObjectList[i]);
			break;
		default:
			break;
		}
