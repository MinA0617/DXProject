...
struct VS3D_INPUT
{
	...
	column_major matrix BoneMat[256]	: BONEMAT;
};
...
VS3D_OUTPUT VS(VS3D_INPUT input)
{
	...
	{
		Output.Position += input.Weight[i] * mul(Pos, input.BoneMat(input.ID[i]));
		Output.Normal += input.Weight[i] * mul(Norm, (float3x3)input.BoneMat(input.ID[i]));
	}
